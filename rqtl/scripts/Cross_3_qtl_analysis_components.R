...
cross_additonal_pheno<-read.table("data/cross_3A_components.tsv", header=T)
...
out_3A.hk_female_flowers <- scanone(cross_obj_sub, pheno.col = 5, method="hk")
operm_3A.hk_female_flowers <- scanone(cross_obj_sub, method="hk", n.perm=1000, pheno.col = 5)
summary(operm_3A.hk_female_flowers)
summary(out_3A.hk_female_flowers)

out_3A.hk_male_flowers <- scanone(cross_obj_sub, pheno.col = 6, method="hk")
operm_3A.hk_male_flowers <- scanone(cross_obj_sub, method="hk", n.perm=1000, pheno.col = 6)
summary(operm_3A.hk_male_flowers)
summary(out_3A.hk_male_flowers)
...
pdf("plots/3A_single_qtl_components.pdf", width=13, height=6)
plot(out_3A.hk_female_flowers, out_3A.hk_male_flowers, lodcolumn = 1, col=c(col_female_flowers, col_male_flowers), ylab="LOD", xlab="")

abline(h=summary(operm_3A.hk_female_flowers)[1], col=col_female_flowers, lty=2, lwd=2)
abline(h=summary(operm_3A.hk_male_flowers)[1], col=col_male_flowers, lty=2, lwd=2)

abline(v=xaxisloc.scanone(out_3A.hk_female_flowers, thechr = summary(out_3A.hk_female_flowers)[7,1], summary(out_3A.hk_female_flowers)[7,2]), col=col_female_flowers, lty=2, lwd=2)
abline(v=xaxisloc.scanone(out_3A.hk_female_flowers, thechr = summary(out_3A.hk_female_flowers)[8,1], summary(out_3A.hk_female_flowers)[8,2]), col=col_female_flowers, lty=2, lwd=2)
